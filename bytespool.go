...
// Package bytespool provides a simple wrapper over sync.Pool for obtaining
// byte slices of different sizes.
package bytespool
...
func (bp *Bpool) Get(sz int, alloc bool) ([]byte, bool) {
	...
	if i >= len(bp.pools) || i < 0 {
		slog.PANIC("pool idx exceeds pool no: %d >= %d,"+
	...
	if (sz > bp.minSz && sz < s) || sz > max {
		slog.PANIC("size does not match: sz %d, pool idx %d"+
...
func (bp *Bpool) Put(buf []byte) bool {
	...
	if i >= len(bp.pools) || i < 0 {
		slog.PANIC("bad pool idx : %d  pool entries %d,"+
